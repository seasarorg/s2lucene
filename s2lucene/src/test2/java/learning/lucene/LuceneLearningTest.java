...
public class LuceneLearningTest {
	...
	/**
	 * 複数のIndexWriterを作成するとエラー IndexのFileLockが発生
	 */
	@Test(expected = RuntimeException.class)
	public void manyCreateIndex() {
		Directory directory = getDirectory(indexPath);
		Analyzer analyzer = new JapaneseAnalyzer(configPath);
		IndexWriter writerFirst = createIndexWriter(directory, false, analyzer);
		IndexWriter writerSecond = createIndexWriter(directory, false, analyzer);
		IndexWriter writerThird = createIndexWriter(directory, false, analyzer);
		close(writerFirst);
		close(writerSecond);
		close(writerThird);
	}
